...
provider "aws" {
  ...
  assume_role {
    role_arn = "arn:aws:iam::${local.account_id}:role/atlantis-assume-roles/atlantis-role-trisolaris"
  }
  default_tags {
    tags = {
      Name = "" # see https://github.com/hashicorp/terraform-provider-aws/issues/19583
...
module "metaflow" {
  ...
  metadata_service_container_image = "794874039740.dkr.ecr.ap-southeast-2.amazonaws.com/metaflow_metadata_service:latest"
